...
- There is an entire field known as Geographical Information Systems (GIS) that we could cover over an entire course.
- It involves complicated problems including how coordinates are obtained, how they can be projected in different ways, etc.
- The `geopandas` and `geoplot` packages can be used to handle these problems for us.
- This package has many dependencies and can be fiddly to install
...
These shapefiles are very detailed making plotting slow. So we can simplify them.
...
- The default projection looks OK for Australia
...
- We can also augment the choropleth with data from individual locations.
...
- You can construct multiple choropleths and display them together but this can be difficult to interpret.
...
- This commonly leads to misinterpratation of data.
...
- However, something like a pairs plot shows more information about the relationships between variables.
...
- Heatmaps are different to choropleths even if these two words are often confused.
